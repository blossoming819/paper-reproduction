...
def tokenize(log):
    ...
    for word in word_lst_tmp:
        ...
        if len(res) == 0:
            ...
            word_lst.extend([w.lower() for w in res])
    return word_lst
...
class Vocab:
    def __init__(self, **kwargs):
        ...
        self.save_dir = kwargs["word2vec_save_dir"] # ../trained_wv
        self.model_type = kwargs["word2vec_model_type"] # fasttext
        self.epochs = kwargs["word2vec_epoch"] # 50
        self.word_window = kwargs["word_window"] # 5
        self.log_lenth = 0
...
class FeatureExtractor(BaseEstimator):
    def __init__(self, **kwargs):
        self.feature_type = kwargs["feature_type"]  # word2vec
        self.data_type = kwargs["data_type"] # fuse log kpi
        self.log_window_size = kwargs["log_window_size"] # 20
        self.model_type = kwargs["word2vec_model_type"] # fasttext naive skip-gram
        self.embedding_dim = kwargs["word_embedding_dim"] # 32
        ...
        for word in word_lst:
            if word in self.known_words:
                ...
                if self.model_type == "naive" or self.model_type == "skip-gram":
                    ...
                    feature.append(self.word_vectors[word])
        return np.array(feature).mean(axis=0).astype("float32")  # [embedding_dim] [token cnts, E]
    ...
    def __seqs2feat(self, seqs):  # seqs in a chunk, with the number of chunk_length
        ...
        if self.feature_type == "sequential":
            # logging.info(str(np.array([self.log2id_train.get(tuple(log), 1) for log in seqs]).shape))
            return np.array([self.log2id_train.get(tuple(log), 1) for log in seqs]).astype("float32")

    def fit(self, chunks):
        total_logs = list(itertools.chain(*[v["logs"] for _, v in chunks.items()])) # 获取所有日志
        self.ulog_train = set(total_logs) # unique logs set
        self.id2log_train = {0: "oovlog"} # Index 0 is reserved for the oov logs.
        self.id2log_train.update({idx: log for idx, log in enumerate(self.ulog_train, 1)}) # Update the map between id and log starting from index 1
        self.log2id_train = {tuple(v): k for k, v in self.id2log_train.items()}
