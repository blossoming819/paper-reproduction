...
class CrossAttention(nn.Module): #k=V
    ...
    # query: [batch_size, output_len, dim]
    # context: [batch_size, query_len, dim]
...
class MultiEncoder(nn.Module):
    def __init__(self, var_nums, device, vocab_size=300, **kwargs):
        super(MultiEncoder, self).__init__()
        self.hidden_size = kwargs["hidden_size"] # 128
...
class MultiModel(nn.Module):
    def __init__(self, var_nums, device, **kwargs):
        ...
        self.fuse_type = kwargs["fuse_type"]
        self.encoder = MultiEncoder(var_nums=var_nums, device=device, **kwargs)
